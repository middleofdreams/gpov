...
DEFAULT_STYLES = {
    ...
    'keyword':      {'foreground': '#600080',
                     'weight': pango.WEIGHT_BOLD},
    'special':      {'foreground': '#381000'},
    'mark1':        {'foreground': '#008B8B'},
    'mark2':        {'foreground': '#6A5ACD'},
    'string':       {'foreground': '#007B06'},
    'number':       {'foreground': '#004B9E'},
...
class UndoableInsert(object):
    ...
    def __init__(self, it, text, length):
        self.offset = it.get_offset()
        self.text = text
        self.length = length
...
class CodeBuffer(gtk.TextBuffer):
	...
	def _on_delete_range(self, buf, start, end):
		...
		if self.not_undoable_action:
			return
		undo_action = UndoableDelete(buf, start, end)
		try:
	...
	def redo(self):
		...
		if isinstance(redo_action, UndoableInsert):
			...
			new_cursor_pos = self.get_iter_at_offset(redo_action.offset + redo_action.length)
